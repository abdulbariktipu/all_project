...
{
	echo create_drop_down( "cbo_location", 172, "select id,location_name from lib_location where status_active =1 and is_deleted=0 and company_id='$data' order by location_name","id,location_name", 1, "-- Select Location --", $selected, "",0 );     	 
}

if ($action=="load_drop_down_store_name")
{
	echo create_drop_down( "cbo_store_name", 172, "select id,store_name from lib_store_name where status_active =1 and is_deleted=0 and company_id='$data' order by store_name","id,store_name", 1, "-- Select Location --", $selected, "",0 );     	 
}

if ($action=="load_drop_down_floor")
{
	echo create_drop_down( "cbo_floor", 170, "select id,floor_name from lib_prod_floor where status_active =1 and is_deleted=0 and location_id='$data' and production_process in (10) order by floor_name","id,floor_name", 1, "-- Select Floor --", $selected, "",0 );     	 
}
...
if($action=="create_po_search_list_view")
{
	...
	{	
 		$sql = "select b.id,a.order_uom,a.buyer_name,a.company_name,a.total_set_qnty,a.set_break_down, a.job_no,a.style_ref_no,a.gmts_item_id,a.location_name,b.shipment_date,b.po_number,b.file_no,b.grouping,b.po_quantity ,b.plan_cut
			...
			b.is_deleted=0 and
			a.garments_nature=$garments_nature
			$sql_cond and b.id in(select po_break_down_id from pro_garments_production_mst where production_type='$qty_source' and status_active=1 and is_deleted=0) order by b.shipment_date"; 
	}
	  //echo $sql;die;
	...
	$total_out_qty=sql_select( "select po_break_down_id, item_number_id, country_id, sum(production_quantity) as production_quantity from pro_garments_production_mst where status_active=1 and is_deleted=0 and production_type=7 group by po_break_down_id, item_number_id, country_id");
	
	foreach($total_out_qty as $row)
	{
		$total_out_qty_data_arr[$row[csf('po_break_down_id')]][$row[csf('item_number_id')]][$row[csf('country_id')]]=$row[csf('production_quantity')];
	}
						...
						?>
							<tr bgcolor="<? echo $bgcolor; ?>" style="text-decoration:none;cursor:pointer" onClick="js_set_value(<? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>');" > 
								...
								<?php
                                 ...
                                 echo $total_cut_qty=$total_out_qty_data_arr[$row[csf('id')]][$grmts_item][$country_id];
								//echo  $row[csf('id')];
                                 ?> &nbsp;
                               </td>
                               <td width="80" align="right">
