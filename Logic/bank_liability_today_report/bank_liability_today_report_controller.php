...
{ 
	...
	group by a.id, a.importer_id, a.issuing_bank_id, a.lc_category, a.lc_value, a.maturity_from_id, a.payterm_id, b.id, b.maturity_date, b.edf_paid_date, b.bank_acc_date, b.nagotiate_date, b.shipment_date, b.bill_date, b.retire_source";
	...
	foreach($ifdbc_edf_sql_result as $row)
	{
		...
		if($row[csf("retire_source")]==30 || $row[csf("retire_source")]==31)
		{
			...
			if($row[csf("payterm_id")]==3) $paid_value=$row[csf("edf_loan_value")];
	...
	$btb_sql="select a.id, a.importer_id, a.issuing_bank_id, a.lc_value from com_btb_lc_master_details a where a.status_active=1 and a.is_deleted=0 and a.ref_closing_status<>1 and to_number(a.lc_category)>0 $sql_cond order by a.importer_id, a.issuing_bank_id";
	        ...
	        <tbody>
	            ...
	            <tr bgcolor="#E9F3FF">
					...
					foreach($all_btb_bank as $com_id=>$com_data)
					{
						...
						{
							?>
	                        <td align="right"><a href='#report_detals'  onclick= "openmypage('<? echo $com_id; ?>','<? echo $bank_id; ?>','ifdbc_popup','IFDBC Info','2')"><? echo number_format($ifdbc_edf_data[$com_id][$bank_id]["ifdbc"],2);?></a>.</td>
	                        <?	
	            ...
	            <tr bgcolor="#FFFFFF">
					...
					foreach($all_btb_bank as $com_id=>$com_data)
					{
						...
						{
							?>
	                        <td align="right"><a href='#report_detals'  onclick= "openmypage('<? echo $com_id; ?>','<? echo $bank_id; ?>','ifdbc_popup','IFDBC Info','3')"><? echo number_format($ifdbc_edf_data[$com_id][$bank_id]["edf"],2);?></a></td>
	                        <?
...
if($action=="btb_popup")
{
	...
	from com_btb_lc_master_details a, com_import_invoice_mst b, com_import_invoice_dtls c
	where a.id=c.btb_lc_id and c.import_invoice_id=b.id and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1 and a.ref_closing_status<>1 and to_number(a.lc_category)>0 and a.importer_id=$company_id and a.issuing_bank_id=$bank_id";
	...
	<fieldset style="width:920px;">
        <table class="rpt_table" border="1" rules="all" width="900" cellpadding="0" cellspacing="0">
			...
			foreach($ifdbc_edf_sql_result as $row)  
			{
				...
				if($type==2)
				{
					$pendin_value=$row[csf("edf_loan_value")]-$invoice_wise_payment[$row[csf("import_inv_id")]];
				}
				else
				{
					$paid_value=0;
					...
					$pendin_value=$row[csf("edf_loan_value")]-$paid_value;
				}
				...
				if($cat_check[$row[csf("lc_category")]*1]=="" && $pendin_value>0)
				{
					...
					{
						?>
						<tr bgcolor="#FFFFCC"><td colspan="9" title="<? echo $row[csf("lc_category")]; ?>"><? echo $supply_source[$row[csf("lc_category")]*1]; ?></td></tr>
						<?
					}
					else
					{
		...
		<fieldset style="width:1040px;">
		    ...
		    <table class="rpt_table" border="1" rules="all" width="1040" cellpadding="0" cellspacing="0" id="table_body">
	            <tbody>
					...
					$main_sql="SELECT a.company_name, a.buyer_name, a.dealing_marchant, b.shiping_status, b.po_break_down_id, c.job_no_mst, c.po_number, sum(b.order_quantity) as po_quantity, sum(b.order_total) as order_total, c.pub_shipment_date
					FROM wo_po_details_master a, wo_po_color_size_breakdown b, wo_po_break_down c 
					WHERE a.job_no=c.job_no_mst and b.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.shiping_status<>3 and c.is_deleted=0 $com_cond 
					GROUP by a.company_name, a.buyer_name, a.dealing_marchant, b.shiping_status, b.po_break_down_id, c.job_no_mst, c.po_number, c.pub_shipment_date 
						...
						if(!in_array($row[csf('dealing_marchant')], $chk))
	                   	{	                   		
	                   		if ($i!=1) 
	                   		{
			                    ?>
			                   	<tr>
			                        <td colspan="5" align="right"><strong>Merchant Total :</strong></td>
                	...
                	foreach ($main_sql_result as $dealing_marchant_key => $row) 
                	{
	                    ...
	                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
	                        ...
	                        if(!in_array($row[csf('buyer_name')], $chk))
		                   	{
			                    ?>
			                        <td width="130" rowspan="<? echo $buyer_rowspan[$row[csf('buyer_name')]]; ?>">
			                        	<? echo $in_hand_qnty_arr[$row[csf('buyer_name')]];
			                        	$total_in_hand_qnty+=$in_hand_qnty_arr[$row[csf('buyer_name')]]; ?>
			                        </td>
			                        <td rowspan="<? echo $buyer_rowspan[$row[csf('buyer_name')]]; ?>">
			                        	<? echo $in_hand_value_arr[$row[csf('buyer_name')]]; 
			                        	$total_in_hand_value+=$in_hand_value_arr[$row[csf('buyer_name')]]; ?>
			                        </td>
			                    <?
	                   		}
...
if ($action=="print_preview_4") // Created by Tipu
{
	...
	$main_sql="SELECT a.company_name, a.buyer_name, a.dealing_marchant, b.shiping_status, b.po_break_down_id, c.job_no_mst, c.po_number, sum(b.order_quantity) as po_quantity, sum(b.order_total) as order_total, to_char(c.pub_shipment_date, 'Month-YY') as monty_year
	FROM wo_po_details_master a, wo_po_color_size_breakdown b, wo_po_break_down c 
	WHERE a.job_no=c.job_no_mst and b.po_break_down_id=c.id and a.status_active=1 and a.is_deleted=0 and b.status_active=1 and b.is_deleted=0 and b.shiping_status<>3 and c.is_deleted=0 $com_cond 
	GROUP by a.company_name, a.buyer_name, a.dealing_marchant, b.shiping_status, b.po_break_down_id, c.job_no_mst, c.po_number, c.pub_shipment_date 
	ORDER by c.pub_shipment_date asc";
	//echo $main_sql;die; // and to_char(a.insert_date,'YYYY') = 2019
	...
	foreach ($main_sql_result as $key => $row) 
   	{
		...
		$in_hand_qnty=$row[csf("po_quantity")]-$Ship_qnty;
   		...
   		$main_arr[$row[csf('buyer_name')]][$row[csf('dealing_marchant')]]=$row[csf('monty_year')];
   		$month_year_qty_arr[$row[csf('buyer_name')]][$row[csf('dealing_marchant')]][$row[csf('monty_year')]]['po_quantity']+=$row[csf("po_quantity")];
   		$month_year_qty_arr[$row[csf('buyer_name')]][$row[csf('dealing_marchant')]][$row[csf('monty_year')]]['inhand_qty']+=$in_hand_qnty;
   		$month_year_qty_arr[$row[csf('buyer_name')]][$row[csf('dealing_marchant')]][$row[csf('monty_year')]]['in_hand_value']+=$in_hand_value; 

   		$month_year_arr[$row[csf('monty_year')]]=$row[csf('monty_year')];
   	}   	
	 	$divWith=count($month_year_arr);
	 	$table_width=800+($divWith*300);
       	/*echo "<pre>";
       	print_r($month_year_arr);die;*/

    ?>
    <div id="report_container" align="center" style="width:810px">
		    ...
		    <table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0">
		        <thead>
		        	<tr>
			            ...
			            <?
	                    foreach($month_year_arr as $monty_year => $row)
	                    {	                    	
                    		?>
	                    	<th width="300" colspan="3"><? echo $monty_year; ?></th>
	                    	<?                    	
	                    }
	                    ?>
                	...
                	foreach ($main_arr as $buyer => $buyer_data) 
                	{
                		...
                		{                			
		                    ...
		                    <tr bgcolor="<? echo $bgcolor; ?>" onClick="change_color('tr_<? echo $i; ?>','<? echo $bgcolor; ?>')" id="tr_<? echo $i; ?>">
			                    ...
			                    foreach($month_year_arr as $monty_year => $row)
			                    {	 
			                    	$po_quantity=$month_year_qty_arr[$buyer][$marcent][$monty_year]['po_quantity'];
			                    	$inhand_qty=$month_year_qty_arr[$buyer][$marcent][$monty_year]['inhand_qty'];
			                    	$in_hand_value=$month_year_qty_arr[$buyer][$marcent][$monty_year]['in_hand_value'];

			                    	$grand_Total_po_quantity[$monty_year]+=$po_quantity;                   	
			                    	$grand_Total_inhand_qty[$monty_year]+=$inhand_qty;                   	
			                    	$grand_Total_in_hand_value[$monty_year]+=$in_hand_value;                   	
			                    	?> 
        	...
        	<table class="rpt_table" border="1" rules="all" width="<? echo $table_width; ?>" cellpadding="0" cellspacing="0" id="report_table_footer">
	        	<tfoot>
	                <tr>
	                    ...
	                    {	                    	
                    		?>
	                    <td width="100"><strong><? echo number_format($grand_Total_po_quantity[$monty_year],2); ?></strong></td>
	                    <td width="100"><strong><? echo number_format($grand_Total_inhand_qty[$monty_year],2); ?></strong></td>
	                    <td width="100"><strong><? echo number_format($grand_Total_in_hand_value[$monty_year],2); ?></strong></td>
	                    <?
		                }
		                ?>
