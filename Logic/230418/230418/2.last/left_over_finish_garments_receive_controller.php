...
if($action=="order_popup")
{
	...
	<script>
		...
		function js_set_value(id,buyer_name,item_id,po_qnty,plan_qnty,country_id)
		{
			//alert(buyer_name);
			//$("#hidden_mst_id").val(id);
			$("#hidden_cbo_buyer_name").val(buyer_name); 
			$("#dd").val(buyer_name); 
			alert(buyer_name);
	...
	?>
     ...
     <div style="width:900px; max-height:240px;overflow-y:scroll;" >	 
		...
		<!-- <? echo $row[csf("id")];?>,'<? echo $grmts_item;?>','<? echo $po_qnty;?>','<? echo $plan_cut_qnty;?>','<? echo $country_id;?>' -->
			<tr onClick="js_set_value('<? echo $row[csf('id')]; ?>','<? echo $buyer_arr[$row[csf('buyer_name')]]; ?>')">
				<td width="30"><? echo $i; ?></td>
				<td width="100"><? echo $row[csf('pub_shipment_date')]; ?></td>
