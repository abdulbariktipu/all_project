...
if($action=="order_popup")
{
	...
	<script>
		...
		function js_set_value(id,buyer_id,country_id,style_ref_no,gmts_item,location_name)
		{
	...
	?>
     ...
     <div style="width:1100px; max-height:240px;overflow-y:scroll;" >	 
		...
		?>
			...
			<tr onClick="js_set_value('<? echo $row[csf('id')]; ?>','<? echo $row[csf('buyer_name')]; ?>','<? echo $country_arr[$row[csf('country_id')]]; ?>','<? echo $row[csf('style_ref_no')]; ?>','<? echo $garments_item[$row[csf('gmts_item_id')]]; ?>','<? echo $row[csf('location_name')]; ?>')">
				<td width="30"><? echo $i; ?></td>
				<td width="100"><? echo $row[csf('pub_shipment_date')]; ?></td>
