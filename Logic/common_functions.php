...
function update_size_sequence($txt_job_no)
{
...
function generate_dynamic_report( $table_id, $tbl_header_arr, $td_width_arr, $tbl_width, $tbl_height, $tbl_border, $query, $onclick_fnc_name, $onclick_fnc_param_db_arr, $onclick_fnc_param_sttc_arr,  $show_sl, $field_printed_from_array_arr,  $data_array_name_arr, $qry_field_list_array, $controller_file_path , $filter_grid_fnc, $fld_type_arr, $summary_flds, $check_box_all )
{
	...
	//echo $_SESSION['logic_erp']["user_name"]	; die;
		...
		function check_magic_quote_gpc( $data )
		{
			...
			//	die;
			if(trim( $data[0]['operation'])=='') $op=99;
			else if($data[0]['operation']==0) $op="save_priv"; 
			else if($data[0]['operation']==1) $op="edit_priv"; 
			else if($data[0]['operation']==2) $op="delete_priv";
			else if($data[0]['operation']==3) $op="approve_priv";
			else if($data[0]['operation']==5) $op=99;
...
// written by :: Fuad
	...
	//}
		...
		{
			$nameArray=sql_select( "select id,project_name,database_name,server_name,ip_address,login_name,login_password,admin_mail,server_id,port_no from lib_db_integration_variables where project_name=$type" );
...
function get_spacial_instruction($mst_id,$width="100%",$entry_form=''){
	...
	<tbody>';
		...
		$data_array=sql_select("select id, terms from  wo_booking_terms_condition where booking_no='".str_replace("'","",$mst_id)."' $entry_form_con ");// quotation_id='$data'
        if ( count($data_array)>0)
        {
        	$i=0;
        	foreach( $data_array as $row )
