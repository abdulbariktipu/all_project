...
<body>
	...
	if ($action=='customer_wise_report') // customer_wise_report
	{
		...
		$comma_sep_sql="SELECT a.orderNumber AS order_no FROM orders a 
		WHERE a.orderNumber=10100 GROUP BY a.orderNumber 
		UNION ALL 
		SELECT a.customerNumber AS order_no FROM customers a, orders b 
		WHERE a.customerNumber=b.customerNumber AND a.customerNumber=124 GROUP BY a.customerNumber";
		$sql_data_result=sql_select($comma_sep_sql);
		$value_arr=array();
		foreach ($sql_data_result as $order_key => $row) 
		{
			$value_arr[$row['order_no']]=$row['order_no'];
		}
		$string = implode(',', array_unique(explode(',', implode(',', $value_arr))));
		// echo "SELECT status FROM orders WHERE orderNumber IN($string) AND customerNumber IN($string)";
		//die;
