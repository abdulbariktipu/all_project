...
function set_conversion_rate($cid, $cdate) {
	...
	if ($cid == 1) {
		...
		//echo $cdate;die;
		$queryText = "select conversion_rate from currency_conversion_rate where con_date<='" . $cdate . "' and currency=$cid and status_active=1 and is_deleted=0 order by con_date desc";
		//echo $queryText; die;
		$nameArray = sql_select($queryText, '', $new_conn);
...
function load_html_head_contents($title, $path, $filter, $popup, $unicode, $multi_select, $am_chart, $jqlatest) 
{
	$html = '
 	...
 	<link href="' . $path . 'css/style_common.css" rel="stylesheet" type="text/css" media="screen" />
 	<link href="' . $path . 'css/dhtmlwindow.css" rel="stylesheet" type="text/css" />
 	...
 	<script src="' . $path . 'popup_window_js/dhtmlwindow.js" type="text/javascript"></script>
 	<script src="' . $path . 'popup_window_js/modal.js" type="text/javascript"></script>';
...
function update_cost_sheet($job_no) //kausar
{
	...
	if (count($data_array) > 0)
	{
		...
		else if ($commercial_cost_method == 5)
		{
			...
			unset($data_yarn);
			$data_array=sql_select("select trims_cost, embel_cost, wash_cost, lab_test, inspection, cm_cost, freight, currier_pre_cost, certificate_pre_cost, design_cost, studio_cost, common_oh from wo_pre_cost_dtls where job_no=$job_no and status_active=1 and is_deleted=0");
			$ft_amount=0;
			foreach( $data_array as $row )
			{
			...
			unset($data_array);
			$fob_value=$ft_amount+$yarn_amount+$fab_amount;
			$amount = def_number_format($fob_value, 5, "");
		}
...
function update_cost_sheet_old($job_no) {
...
function signature_table($report_id, $company, $width, $template_id, $padding_top = 70,$prepared_by='') {
	if ($template_id != '') {
		$template_id = " and template_id=$template_id ";
	}
	$sql = sql_select("select designation,name,activities,prepared_by from variable_settings_signature where report_id=$report_id and company_id=$company  $template_id order by sequence_no");


	if($sql[0][csf("prepared_by")]==1){
		$sql_2[100] = array ( DESIGNATION => 'Prepared By' ,NAME => $prepared_by, ACTIVITIES =>'', PREPARED_BY => 0 );
		$sql=$sql_2+$sql;
	}
...
function get_spacial_instruction($mst_id, $width = "100%", $entry_form = '') {
	...
	$data_array = sql_select("select id, terms from  wo_booking_terms_condition where booking_no='" . str_replace("'", "", $mst_id) . "' $entry_form_con ");
	if (count($data_array) > 0) {
		$i = 0;
		foreach ($data_array as $row) {
			$i++;
...
{
	return "<h1 style=\"color:red;font-size:20px;\">No Data Found.</h1>";
}
?>