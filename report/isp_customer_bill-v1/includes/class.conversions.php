...
class conversion extends report{
	...
	private function _setQuery(){
		...
		$this->_query="select a.job_no,b.id,c.item_number_id,c.country_id,c.color_number_id,c.size_number_id,c.order_quantity ,c.plan_cut_qnty ,d.id as pre_cost_dtls_id,d.fab_nature_id,d.color_size_sensitive,e.cons,e.requirment,f.id as convertion_id,f.fabric_description,f.cons_process,f.req_qnty,f.avg_req_qnty,f.charge_unit,f.amount,f.color_break_down   from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_fabric_cost_dtls d,wo_pre_cos_fab_co_avg_con_dtls e,wo_pre_cost_fab_conv_cost_dtls f  where 1=1 ".$jobcond." ".$pocond."  and a.job_no=b.job_no_mst and a.id=c.job_id and a.id=d.job_id and a.id=e.job_id and a.id=f.job_id and b.id=c.po_break_down_id and d.id=e.pre_cost_fabric_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= d.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and d.id=f.fabric_description  and e.cons !=0   and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1
UNION ALL
select a.job_no,b.id,c.item_number_id,c.country_id,c.color_number_id,c.size_number_id,c.order_quantity ,c.plan_cut_qnty ,d.id as pre_cost_dtls_id,d.fab_nature_id,d.color_size_sensitive,e.cons,e.requirment,f.id as convertion_id,f.fabric_description,f.cons_process,f.req_qnty,f.avg_req_qnty,f.charge_unit,f.amount,f.color_break_down   from wo_po_details_master a, wo_po_break_down b,wo_po_color_size_breakdown c,wo_pre_cost_fabric_cost_dtls d,wo_pre_cos_fab_co_avg_con_dtls e,wo_pre_cost_fab_conv_cost_dtls f  where 1=1 ".$jobcond." ".$pocond."  and a.id=b.job_id and a.id=c.job_id and a.id=d.job_id and a.id=e.job_id and a.id=f.job_id and b.id=c.po_break_down_id and d.id=e.pre_cost_fabric_cost_dtls_id and c.po_break_down_id=e.po_break_down_id and c.item_number_id= d.item_number_id and c.color_number_id=e.color_number_id and c.size_number_id=e.gmts_sizes and f.fabric_description=0  and e.cons !=0  and a.is_deleted=0 and a.status_active=1 and b.is_deleted=0 and b.status_active=1 and c.is_deleted=0 and c.status_active=1";

		$this->_query2="select a.id as convertion_id, a.color_break_down from wo_pre_cost_fab_conv_cost_dtls a where 1=1 ".$jobcond."";
		$this->_query3="select g.pre_cost_fabric_cost_dtls_id , g.gmts_color_id , g.contrast_color_id  from wo_po_details_master a,wo_pre_cos_fab_co_color_dtls g where 1=1 ".$jobcond." and a.id=g.job_id";
	}
	...
	public function _setCotrastColorArray(){
		foreach($this->_dataArray3 as $row){
			...
			$contrast_color_id=$row[csf('contrast_color_id')];
	...
	private function _setAmount($level){
		...
		foreach($this->_dataArray as $row)
		{
			...
			$fabNatureId=$row[csf('fab_nature_id')];
			$colorSizeSensitive=$row[csf('color_size_sensitive')];
			$cons=$row[csf('cons')];
			$requirment=$row[csf('requirment')];
