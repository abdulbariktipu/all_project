...
function check_ext(ext,type)
{
	...
	{
	 	//if (! (ext && /^(pdf|txt|doc|docx|xls|xlsx|csv|msg)$/.test(ext))) return false;
	...
	$(function(){
		...
		new AjaxUpload(btnUpload, {
			...
			onComplete: function(file, response){
				...
				if(file_type==1)
				{
					$('<li></li>').appendTo('#files').html('<a target="_blank" href="../'+response[0]+'"><img src="../'+response[0]+'" height="97" width="89" /></a><br />'+'<p>'+response[6]+'<p>'+del_file).addClass('success');
				...
				{
					$('<li></li>').appendTo('#files').html('<a href="common_functions_for_js.php?filename=../'+trim(response[0]) +'&action=download_file"> <img src="../file_upload/blank_file.png" height="97px" width="89px" /></a><br />'+'<p>'+response[6]+'<p>'+del_file).addClass('success');
				}
			}
		});
	});
        	...
        	<tr>
                ...
                <?
					...
					?>			<li><a href="##" > <img  src="<? echo '../'. $inf[csf("image_location")]; ?>" height="97" width="89" /></a><br />
								<? if($show_button==1) {
									?>
									<p><? echo $inf[csf("real_file_name")]; ?></p>
									<a href='##'  onClick="remove_this_image('<? echo $inf[csf("id")]; ?>','<? echo $lin; ?>','<? echo $inf[csf("master_tble_id")]; ?>','<? echo $inf[csf("details_tble_id")]; ?>','<? echo $inf[csf("form_name")]; ?>','<? echo $inf[csf("file_type")]; ?>')"><? echo "Delete"; //echo $inf[csf("real_file_name")]; ?></a>  
									<? } ?> 	 	
								</li>
                ...
                <? 
					...
					?>			<li><a href="common_functions_for_js.php?filename=<? echo "../".$lin ?>&action=download_file" style="text-transform:none"> <img  src="<? echo '../file_upload/blank_file.png'; ?>" height="97" width="89" /></a><br />
								<? if($show_button==1) {
									?>
									<p><? echo $inf[csf("real_file_name")]; ?></p>
									<a href='##' onClick="remove_this_image('<? echo $inf[csf("id")]; ?>','<? echo $lin; ?>','<? echo $inf[csf("master_tble_id")]; ?>','<? echo $inf[csf("details_tble_id")]; ?>','<? echo $inf[csf("form_name")]; ?>','<? echo $inf[csf("file_type")]; ?>')"><? echo "Delete";//$inf[csf("real_file_name")]; ?></a>  
									<? } ?> 	 	
								</li>
	...
	if($db_type==2 || $db_type==1)
	{
		...
		if (count($nameArray)>0) 
		{
			foreach ($nameArray as $inf)
			{
				...
				{
				$new_img .="<li><a href='common_functions_for_js.php?filename=../$lin&action=download_file' style='text-transform:none' > <img  src='../file_upload/blank_file.png' height='97' width='89' /></a><br /><p>".$inf[csf("real_file_name")]."</p>
				<a href='##' onclick=\"remove_this_image('".$inf[csf("id")]."','".$inf[csf("image_location")]."','".$inf[csf("master_tble_id")]."','".$inf[csf("details_tble_id")]."','".$inf[csf("form_name")]."','".$inf[csf("file_type")]."')\">"."Delete"."</a>
				</li>";	
				}
			}
