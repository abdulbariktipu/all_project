...
class Register_Controller extends CI_Controller 
{
    ...
    public function contact_as_function()
	{
		$this->load->view("contact_as"); // load contact_as.php
	}

	public function save_contact_info()  //insert contact_as page data into database table.
    {
    	//echo "save_contact_info";die;
        //this array is used to get fetch data from the view page.  
        $data = array(
                        'name'     	=> $this->input->post('InpuEame'),  
                        'email'  	=> $this->input->post('InputEmail'),  
                        'telephone' => $this->input->post('telephone'),  
                        'message' 	=> $this->input->post('MessageDescription') 
                    );
        //print_r($data);die;
    	if (!$data['name']=='' && !$data['email']=='')
		{
        	$this->load->model('U_Model'); 
        	if ($this->U_Model->CheckExitContact_info($data['email'])) // Data Already Exit, more comma separetor
        	{
        		//echo CheckExitContact_info();die;
        		$this->session->set_flashdata('emsg','Data Already Exit');
				redirect("Register_Controller/contact_as_function"); 
        	}
        	else
        	{
        		$this->U_Model->contact_info_save_to_database($data);  	  
        		$this->session->set_flashdata('smsg','Data Save Success');
				redirect("Register_Controller/contact_as_function"); 
        	}	       
		}
		...
		{
			$this->session->set_flashdata('emsg','Name and email can not be blank');
			redirect("Register_Controller/contact_as_function"); 
		}

		// Data Show 2 start contact_as page
		/*$query = $this->U_Model->showMessage(); 
		$data['query'] = null;
		if($query)
		{
			$data['query'] =  $query;
