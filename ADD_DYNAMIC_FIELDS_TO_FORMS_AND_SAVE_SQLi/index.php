...
<head>
    ...
    <script>
            ...
            var total = 0;         
		      	for (var j=1; j<=row_num; j++)
    				{    
    					...
    					multi_data+="&txt_item_account_" + j + "='" + $('#txt_item_account_'+j).val()+"'&txt_item_dtls_" + j + "='" + $('#txt_item_dtls_'+j).val()+"'&txt_qty_" + j + "='" + $('#txt_qty_'+j).val()+"'";
    				}
      ...
      function function_name(argument) 
      {
          ...
          for (var j=1; j<=row_num; j++)
          {
            $("#dynamic_field input[type=checkbox]:checked").each(function (e) {
        ...
        {
          var ischecked= $('#chk_'+i).prop('checked');
          if (ischecked==true) {
            qty += parseInt($('#txt_qty_'+i).val());
            var req_qty = 10;
            if (req_qty <= qty) 
            {
              alert('qty is full='+qty);
            }            
          }
        }
      ...
      /*function function_name(argument) {
        ...
        {
          if(argument[i]) ($('input:checkbox').prop('checked')) {
            alert('ok');
          }
        }
      }*/
         ...
         <form name="add_name" id="add_name">
            ...
            <div class="table-responsive">  
                <table class="table table-bordered" id="dynamic_field">  
                    <tr>
                        <td><input type="checkbox" name="chk[]" id="chk_1" class="form-control name_list" onclick="function_name(1)"></td>
                        ...
                        <td><input type="text" name="txt_qty_1" id="txt_qty_1" placeholder="Iteme Qty" class="form-control name_list" ></td>
    ...
    $(document).ready(function()
    {
      var i=1;
      	...
      	$('#added').click(function(){
           i++;
           $('#dynamic_field').append('<tr id="row'+i+'" class="dynamic-added"><td><input type="checkbox" name="chk[]" id="chk_'+i+'" class="form-control name_list" onclick="function_name('+i+')"/></td><td><input type="text" name="txt_item_account_'+i+'" id="txt_item_account_'+i+'" placeholder="Enter your Name" class="form-control name_list" required /></td><td><input type="text" name="txt_item_dtls_'+i+'" id="txt_item_dtls_'+i+'" placeholder="Iteme Dtls" class="form-control name_list" required /></td><td><input type="text" name="txt_qty_'+i+'" id="txt_qty_'+i+'" placeholder="Iteme Qty" class="form-control name_list" required /></td><td><button type="button" name="remove_field" id="'+i+'" class="btn btn-danger btn_remove">X</button></td></tr>');  
      	});
