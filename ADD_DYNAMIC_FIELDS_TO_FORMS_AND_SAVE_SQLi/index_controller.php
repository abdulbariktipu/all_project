<?php
	//error_reporting (0);
	$conn = mysqli_connect('localhost','root','','addmore');
	...
	if ($action=='save_update_delete_dtls') 
	{
		...
		$query = mysqli_query($conn, "SELECT * FROM inventory WHERE make in ($unique)");
		...
		{
			$insert = mysqli_query($conn, "INSERT INTO inventory ($field_array) VALUES $data_array"); 
			if (!$insert) 
			{
